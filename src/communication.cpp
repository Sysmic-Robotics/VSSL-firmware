...
static void clearCommandInputs() {
    g_Input_X = 0;
    g_Input_Y = 0;
}
...
void updateCommunication() {
        ...
        // Si no llegan paquetes en cierto tiempo, detener comandos
        if (communicationConnected && (millis() - lastCommandTime > COMM_TIMEOUT_MS)) {
            clearCommandInputs();
            communicationConnected = false;
        }
...
bool isCommunicationConnected() {
    #ifdef MODO_BASESTATION
        return communicationConnected && (millis() - lastCommandTime <= COMM_TIMEOUT_MS);
    #else
